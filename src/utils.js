...
// Функция для отображения стоимости в локальном формате
export function displayPrice(value, currency = "RUB", locale = "ru-RU") {
...
// Функция для расчета итоговой стоимости корзины с указанием в локальном формате
export function calculateCartTotal(cart) {
  const cartSum = cart.reduce(
    (sum, { price, amount }) => sum + price * amount,
    0
  );
  return displayPrice(cartSum);
}
